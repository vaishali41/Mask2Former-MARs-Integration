...
def save_eigencam_visualization(model, image_tensor, original_image_tensor, 
    ...
    def tensor_to_numpy(tensor):
        ...
        # Check if image is already in 0-255 range or 0-1 range
        if img.max() <= 1.0:
            # Likely in 0-1 range, scale to 0-255
            img = img * 255.0
        elif img.min() < 0:
            # Likely normalized, denormalize
            # Try detectron2's normalization (BGR format)
            mean = np.array([103.530, 116.280, 123.675])  # BGR
            std = np.array([57.375, 57.120, 58.395])      # BGR
            img = img * std + mean
            # Convert BGR to RGB
            img = img[:, :, ::-1]
    ...
    try:
        ...
        print(f"   ⚠️ Error converting images: {e}")
        # Fallback: just scale to 0-255
        original_np = (original_image_tensor.cpu().numpy().transpose(1, 2, 0) * 255).clip(0, 255).astype(np.uint8)
        rotated_np = (image_tensor.cpu().numpy().transpose(1, 2, 0) * 255).clip(0, 255).astype(np.uint8)
    ...
    # Create a combined figure with better layout
    ...
    if cam_original is not None:
        im2 = ax2.imshow(cam_original, cmap='jet', vmin=0, vmax=1)
        ax2.set_title('EigenCAM (Original)', fontweight='bold', fontsize=14)
        plt.colorbar(im2, ax=ax2, fraction=0.046, pad=0.04)
    else:
        ax2.text(0.5, 0.5, 'CAM not available', 
                ha='center', va='center', transform=ax2.transAxes, fontsize=12)
        ax2.set_title('EigenCAM (Original)', fontweight='bold', fontsize=14)
    ax2.axis('off')
    
    ax3 = fig.add_subplot(gs[0, 2])
    ax3.imshow(overlay_original)
    ax3.set_title('Overlay (Original)', fontweight='bold', fontsize=14)
    ax3.axis('off')
    
    # Row 2: Rotated image
    ax4 = fig.add_subplot(gs[1, 0])
    ax4.imshow(rotated_np)
    ax4.set_title(f'Rotated Image ({rotation_angle}°)', fontweight='bold', fontsize=14)
    ax4.axis('off')
    
    ax5 = fig.add_subplot(gs[1, 1])
    if cam_rotated is not None:
        im5 = ax5.imshow(cam_rotated, cmap='jet', vmin=0, vmax=1)
        ax5.set_title(f'EigenCAM ({rotation_angle}°)', fontweight='bold', fontsize=14)
        plt.colorbar(im5, ax=ax5, fraction=0.046, pad=0.04)
    else:
        ax5.text(0.5, 0.5, 'CAM not available',
                ha='center', va='center', transform=ax5.transAxes, fontsize=12)
        ax5.set_title(f'EigenCAM ({rotation_angle}°)', fontweight='bold', fontsize=14)
    ax5.axis('off')
    
    ax6 = fig.add_subplot(gs[1, 2])
    ax6.imshow(overlay_rotated)
    ax6.set_title(f'Overlay ({rotation_angle}°)', fontweight='bold', fontsize=14)
    ax6.axis('off')
